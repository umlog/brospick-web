...
import ProductDetailClient from '../apparel/[slug]/ProductDetailClient';

export function buildProductMetadata(slug: string): Metadata {
...
/** 의류(/apparel)·부츠스킨(/bootskin) 상세 페이지가 공유하는 본문 */
export default async function ProductDetailPage({ slug }: { slug: string }) {
  const product = products[slug as ProductSlug];

  if (!product) {
    return <ProductDetailClient params={{ slug }} initialPrice={null} initialSizes={[]} initialReviews={[]} initialAvgRating={0} initialReviewCount={0} />;
  }
