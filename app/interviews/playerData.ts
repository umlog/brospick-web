...
export const playersData: Record<string, Player> = {
  'kim-geono': {
    ...
    profileImage: '/players/guno/action-1.jpg',
  },
  'choi-eunwoo': {
    id: 'choi-eunwoo',
    basicInfo: {
    ...
    interview: '안녕하세요, 호주 축구 선수 최은우입니다. 국내에서 조선대 평택 시티즌에서 선수 생활을 이어갔지만 그 이상의 상위리그 진출에 어려움을 겪고 있었습니다. 독립 구단에서도 해외 진출을 위해 노력하였지만, 한국에서의 좋은 기회와 필수적인 구단들의 테스트 시에는 한계가 있다고 판단하였습니다. 혼자 에이전시 없이 해외 진출을 위해 많은 비용을 내야 해서 무리가 있었지만 팀 브로스픽을 통해서 저 장점과 하이라이트 영상 등의 어려움 등 더 높은 무대인 호주 NPL Tigers FC에 이상적인 조건으로 입단할 수 있었습니다. 저처럼 어려움을 겪는 선수가 있다면 브로스픽을 통해서 새로운 기회를 잡아 보셨으면 좋겠습니다! 감사합니다.',
